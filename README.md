...
5. **AI 데이터 정제 및 분석 (신규)**: 크롤링된 공고 본문과 첨부파일(PDF, HWP 등)을 **Gemini 2.5 Flash** 멀티모달 AI를 사용해 종합 분석하고, 서비스에 맞게 정형화된 JSON 데이터를 추출하여 데이터베이스(`processed_funding_records` 테이블)에 최종 저장합니다.
